...
uint64_t first;
uint64_t period_raw;  // raw value of the period count, it is updated with timer interrupt

void initialize_pulse_timer()
{
    first = 0xFFFFFFFF;
    //WTIMER1 - timerB will be used in Edge Time Mode with prescalar, PC7
    ...
    GPIOPinTypeTimer(GPIO_PORTC_BASE, GPIO_PIN_7); //Use PD0 as CCP pin
    ...
    TimerConfigure(WTIMER1_BASE, TIMER_CFG_B_CAP_TIME);  // TimerB will count down
...
void read_period_raw()
{
    ...
    period_raw = (first-TimerValueGet(WTIMER1_BASE, TIMER_B))&0xFFFFFFFF;  // Calculate the period and mask it to prevent overflow errors
    first = TimerValueGet(WTIMER1_BASE, TIMER_B);  // Take last time to use in future
    convert_period_to_velocity();
}

void convert_period_to_velocity(void)
{
    UARTprintf("%u \n", period_raw);
    // Since both high and low side is measured, measured time is 0.5 of the 1 pulse distance
    /*float angle = (encod->countToDegree)/2.0f;
    encod->velocity = angle * 80000000.0f / (float)period_raw ;
    return encod->velocity;*/
}
